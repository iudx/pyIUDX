...
class Catalogue():
    ...
    def getResourceItemsFor(self, attribute_name, attribute_value):
        url = self.get_url() + "/search?attribute-name=(" + attribute_name + \
...
class Auth():
    ...
    def call(self, api, body=None):
        ...
        if api == "acl":
            ...
            connect = requests.post(self.url + "/" + api, cert=self.credentials,
                                    verify=True, data=body, headers={"content-type": "application/json"})
...
class resourceServer():
    ...
    def getLatestData(self, serverDomain, serverPort, serverVersion, resource):
        ...
        url = self._serverDomain+":"+self._serverPort+"/api/" + \
            self._serverVersion+"/resource/latest/"+self._resource
